...
const aligns = [
  { value: 'left', icon: <AlignTextLeft /> },
  { value: 'center', icon: <AlignTextCenter /> },
  { value: 'right', icon: <AlignTextRight /> },
  { value: 'both', icon: <AlignTextBoth /> },
];
...
const AlignButton = () => {
  ...
  const popContent = useMemo(() => {
    return (
      <div className="align-pop-content">
        {aligns.map((item) => {
          return (
            <BaseButton
              selected={item.value === alignValue}
              icon={item.icon}
              onMouseDown={() => {
